//refer: http://blog.csdn.net/vonzhoufz/article/details/37662581
...
int main(int argc, char *argv[])
{
    ...
    maxfd = listenfd;
    maxi = -1;
    for(i= 0; i < FD_SETSIZE; i++) connectfd[i] = -1;
    ...
    while(1)
    {
        ...
        if(FD_ISSET(listenfd, &read_fds))
        {
            ...
            if(i > maxi) maxi = i;// update the max index in connectfd[]
        ...
        for(i = 0; i <= maxi; i++)
        {
            ...
            if((sockfd = connectfd[i]) < 0) continue;
            ...
            for(i = 0; i <= maxi; i++)
            {
                ...
                {
                    //handle exceptions  //get out of band data 
